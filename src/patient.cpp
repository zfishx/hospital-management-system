...
void patient::saveMap()
{
    ...
    f << "patientId,firstName,lastName,gender,age,mobNumber,address,height,weight,wasHospitalized?,stillAlive(ifHospitalized)?,password\n";
    for (auto i : hospital::patientsList)
        f << i.second.id << "," << i.second.firstName << "," << i.second.lastName << "," << i.second.gender
          ...
          << (i.second.hospitalized ? 'Y' : 'N') << "," << (i.second.alive ? 'Y' : 'N') << "," << i.second.password << endl;
    f.close();
...
void patient::hospitalize()
{
    ...
    if (!hospital::patientsList[id].hospitalized){
        ...
        for(auto i : hospital::bedsList){
            //printf("bed occupied: %d\n", i.second.getisOccupied());
            if(!i.second.getisOccupied()){
                i.second.setpatientId(id);
                i.second.setisOccupied(true);
                hospital::bedsList[i.second.getId()] = i.second;
                cout<<"\nBed "<<i.second.getId()<<" allocated to "<<firstName<<" "<<lastName<<"!\n";
                flag = 1;
                hospital::patientsList[id].hospitalized = true;
                break;
            }
        }
        if(!flag){
...
void patient::reportADeath()
{
    ...
    if (!hospital::patientsList[id].hospitalized){
        ...
        for(auto i : hospital::bedsList){
            if(i.second.getpatientId() == id){
                i.second.setpatientId(-1);
                i.second.setisOccupied(false);
                hospital::bedsList[i.second.getId()] = i.second;
                cout<<"\nBed "<<i.second.getId()<<" deallocated!\n";
                break;
            }
        }
        hospital::patientsList.erase(id);
